...
export function extractYouTubeVideoId(input: string): string | null {
  ...
  // URLs: watch?v=, embed/, youtu.be/, shorts/, live/
...
export function extractYouTubeChannelIdentifier(input: string): { handle?: string; channelId?: string; customName?: string } | null {
  ...
  if (trimmed.startsWith('@')) {
    return { handle: trimmed.substring(1) };
  }
  ...
  const customMatch = trimmed.match(/(?:https?:\/\/)?(?:www\.)?youtube\.com\/(?:c|user)\/([a-zA-Z0-9_.-]+)/i);
  if (customMatch && customMatch[1]) {
    return { customName: customMatch[1] };
  }
...
/**
 * Fetch real YouTube oEmbed metadata from YouTube official servers
 */
...
export async function resolveTargetMetadata(inputUrlOrQuery: string): Promise<ResolvedMetadata> {
  ...
  if (!query) {
    throw new Error('Input URL or search query is required');
  }

  // 1. Check if it's a YouTube Video
  ...
  if (videoId) {
    ...
    try {
      ...
      // Direct YouTube format fallback with real image URL
      return {
        type: 'video',
        platform: 'YouTube',
  ...
  if (channelInfo) {
    const channelName = channelInfo.handle ? `@${channelInfo.handle}` : channelInfo.customName || channelInfo.channelId || 'YouTube Channel';
    const channelUrl = channelInfo.handle 
      ...
      : `https://www.youtube.com/${channelInfo.customName}`;
  ...
  // 3. TikTok / Instagram / X / Facebook links
  ...
  // 4. Fallback search by creator handle or general search query
  const cleanQuery = query.replace(/^@/, '');
  return {
    type: 'channel',
    platform: 'YouTube',
    title: `${query} Official Channel`,
    channelName: query.startsWith('@') ? query : `@${cleanQuery}`,
    channelUrl: `https://www.youtube.com/@${cleanQuery}`,
    thumbnailUrl: 'https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?w=150&auto=format&fit=crop&q=80',
  };
}
